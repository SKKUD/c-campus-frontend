...
const QuizBox = ({Quiz, Answer}: IQuizBox) => {
  ...
  useEffect(() => {
    if (!isHint) { // 힌트가 나오는게 아닐 때, -> 몇 번 틀렸는지 말해주는 것 일 때
      SetColor(RED_COLOR);
      SetInformation(`틀렸습니다! ${wrongCount}번의 시도 후에 힌트가 공개됩니다.`);
    }
  }, [wrongCount]);
  ...
  return (
    <QuizBoxContainer>
      ...
      {
        (wrongCount < 3) ? (<QuizBoxInformation color={color}>{information}</QuizBoxInformation>) : (<div></div>)
      }
